...
if (isset($_POST['login'])) {
    ...
    if (empty($UserID)) {
        ...
        $query = "SELECT * FROM FacultyLogin WHERE LOWER(UserID) = LOWER('$UserID')";
        ...
        if ($count > 0) {
            ...
            $query = "INSERT INTO FacultyLogin (UserID, UserName, Password) VALUES ('$UserID','$UserName','$Password')";
            if (mysqli_query($con, $query)) {
                echo "<script>alert('Faculty Registration successful')</script>";
            } else {
                echo "<script>alert('Faculty Registration failed')</script>";
            }
        }
    }
}
