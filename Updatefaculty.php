...
if (isset($_POST['login'])) {
  ...
  if (empty($UserID)) {
    echo "<script>alert('FacultyID is empty')</script>";
  } else {
    $result = mysqli_query($con, "SELECT * FROM facultylogin WHERE UserID='$UserID'");

    if (mysqli_num_rows($result) == 0) {
      echo "<script>alert('Faculty with ID $UserID not found')</script>";
      ...
      if ($userOption == "name") {
        ...
        if (empty($UserName)) {
          ...
          $query = "UPDATE facultylogin SET UserName='$UserName' WHERE UserID='$UserID'";
          $fieldName = "UserName";
        }
        ...
        if (empty($Password)) {
          ...
          $query = "UPDATE facultylogin SET Password='$hashedPassword' WHERE UserID='$UserID'";
          $fieldName = "Password";
        }
      }
